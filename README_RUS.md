...
- `enter/=` - подсчет математического выражения;
- `esc` - очистить поле и историю;
...
- `v` - перейти в режим ввода подсчета математического выражения с переменной x;
...
9. В режиме ввода переменной подсчет выражения не произведется, если в выражении отсутствует переменная `x` или если введено некорректное значение `x`;
...
12. Доступен ввод необходимого шага графика для построения более точных графиков. Диапазон допустимого шага варъируется в `{0.001:100}` (следует также отметить, что при большой области допустимых значений, то есть диапазоне `x` - чем меньше шаг, тем дольше выполняется построение);
...
2. Интерактивно валидируется все поля пользовательского ввода;
...
8. При нажатии на кнопку `Calculate` формируется полный отчет о кредите, содержащий информацию о каждом месяце кредита и общие результаты выплаченной суммы.
...
9. При нажатии на кнопку `Calculate` формируется полный отчет о депозите, содержащий информацию о каждом периоде капитализации счета, общие результат по выплаченным суммам и суммам налогообложений.
