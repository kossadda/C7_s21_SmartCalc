...
class credit : public QMainWindow {
 ...
 private slots:
  ...
  int check_date_between(const QDate &previous, const QDate &current,
                         int *redemp_count);
  void add_datarow_to_table(const QDate &date, QString row_head,
                            long double **result, int iteration, int style);
