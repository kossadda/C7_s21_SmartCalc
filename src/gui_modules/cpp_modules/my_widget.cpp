...
my_widget::my_widget(QWidget *parent)
    : QWidget(parent),
      ...
      wrong_style(
          "QLineEdit {border-radius: 10px;padding: 0 8px;background-color: "
          "rgba(255, 0, 0, 100);color:black;border: 1px solid black;}"),
      ui(new Ui::my_widget) {
  ui->setupUi(this);
  ui->table->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
  ui->table->horizontalHeader()->setStyleSheet(
      "background-color: rgb(226, 226, 226); color: rgb(0, 0, 0);");
  ...
  connect(ui->pay_sum, SIGNAL(textChanged(QString)), this,
          SLOT(onOperEditTextChanged(QString)));
}

my_widget::~my_widget() { delete ui; }

void my_widget::on_add_to_table_clicked() {
  if (ui->pay_sum->text().length() &&
      onOperEditTextChanged(ui->pay_sum->text()) == 0) {
...
QTableWidget *my_widget::getTableWidget() { return ui->table; }

QComboBox *my_widget::getUi_type() { return ui->pay_type; }

QLineEdit *my_widget::getUi_sum() { return ui->pay_sum; }
