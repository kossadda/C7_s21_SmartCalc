...
credit::credit(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::credit), tableWindow(nullptr) {
  ui->setupUi(this);
  ...
  connect(ui->creditBox, &QComboBox::currentTextChanged, this,
          &credit::change_mode);

  connect(ui->amount_edit, SIGNAL(textChanged(QString)), this,
          SLOT(onAmountEditTextChanged(QString)));
  connect(ui->time_edit, &QLineEdit::textChanged, this,
          &credit::onTimeEditTextChanged);
  connect(ui->time_box, SIGNAL(currentIndexChanged(int)), this,
          SLOT(onTimeEditIndexChanged()));
  connect(ui->percent_edit, &QLineEdit::textChanged, this,
          &credit::onInterestTextChanged);
}
...
void credit::change_mode(const QString index) {
...
void credit::onTimeEditIndexChanged() {
  onTimeEditTextChanged(ui->time_edit->text());
}

int credit::onAmountEditTextChanged(const QString &text) {
...
int credit::onTimeEditTextChanged(const QString &text) {
  int valid = WRONG_EXPR;
...
int credit::onInterestTextChanged(const QString &text) {
  int valid = WRONG_EXPR;
...
void credit::free_memory(credit_init data, payments *pay,
...
void credit::add_redemption(early_pay *redemption, my_widget *early_pays,
                            int count) {
  redemption->date =
      (time_data *)realloc(redemption->date, (count + 1) * sizeof(time_data));
  redemption->sum = (long double *)realloc(redemption->sum,
                                           (count + 1) * sizeof(long double));
  redemption->type =
      (int *)realloc(redemption->type, (count + 1) * sizeof(int));
...
void credit::init_parameters(credit_init *data, Ui::credit *ui) {
...
int credit::init_redemption(early_pay *redemption) {
...
void credit::on_calculate_clicked() {
  ...
  if (early_pays) {
    ...
    for (int count = 0; count < early_pays->getTableWidget()->rowCount();
         count++) {
...
void credit::add_item_to_table(int row, int column, QString value, int style) {
...
int credit::check_date_between(const QDate &previous, const QDate &current,
                               int *redemp_count) {
  QDate one_date;
  QTableWidgetItem *redem_item =
      early_pays->getTableWidget()->item(*redemp_count, 0);
  one_date = QDate::fromString(redem_item->text(), "dd.MM.yyyy");
  return (one_date >= previous && one_date < current) ? 1 : 0;
}

void credit::add_datarow_to_table(const QDate &date, QString row_head,
...
void credit::add_all_to_table(int months, long double **result,
                              long double *total) {
  int redemp_count = 0;
  ...
  for (int i = 0; i < months; i++) {
    ...
    add_datarow_to_table(current_day, QString::number(i + 1 - redemp_count),
                         result, i, 0);
    ...
    if (current_day.day() != first_date) {
      if (first_date > current_day.day()) {
        current_day.setDate(current_day.year(), current_day.month(),
                            current_day.daysInMonth());
      } else {
        current_day.setDate(current_day.year(), current_day.month(),
                            first_date);
      }
