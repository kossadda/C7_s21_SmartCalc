    ...
    , tableWindow(nullptr)
{
    ...
    connect(ui->creditBox, &QComboBox::currentTextChanged, this, &credit::change_mode);

    connect(ui->amount_edit, SIGNAL(textChanged(QString)), this, SLOT(onAmountEditTextChanged(QString)));
    connect(ui->time_edit, &QLineEdit::textChanged, this, &credit::onTimeEditTextChanged);
    connect(ui->time_box, SIGNAL(currentIndexChanged(int)), this,SLOT(onTimeEditIndexChanged()));
    connect(ui->percent_edit, &QLineEdit::textChanged, this, &credit::onInterestTextChanged);
}
...
void credit::free_memory(credit_init data, payments *pay, early_pay *redemption) {
...
void credit::add_redemption(early_pay *redemption, my_widget *early_pays, int count) {
...
int credit::check_date_between(const QDate& previous, const QDate& current, int* redemp_count) {
    QDate one_date;
...
void credit::add_datarow_to_table(const QDate& date, QString row_head, long double **result, int iteration, int style) {
...
void credit::add_all_to_table(int months, long double **result, long double *total) {
    int redemp_count = 0;
