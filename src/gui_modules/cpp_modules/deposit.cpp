...
deposit::deposit(QWidget *parent) : QMainWindow(parent), ui(new Ui::deposit) {
  ...
  tableWindow->getUi()->table->horizontalHeader()->setSectionResizeMode(
      QHeaderView::Stretch);
  ...
  taxWindow->horizontalHeader()->setStyleSheet(
      "background-color: rgb(255, 255, 255); color: rgb(0, 0, 0)");
  taxWindow->verticalHeader()->setStyleSheet(
      "background-color: rgb(255, 255, 255); color: rgb(0, 0, 0)");
  ...
  QString list_style =
      "QListView {background-color: white;} QListView::item {border-bottom: "
      "5px solid white; margin:3px; background-color: white; } "
  ...
  connect(ui->capital, SIGNAL(stateChanged(int)), this,
          SLOT(onCheckBoxStateChanged(int)));
  connect(ui->amount_edit, SIGNAL(textChanged(QString)), this,
          SLOT(onAmountEditTextChanged(QString)));
  connect(ui->time_edit, SIGNAL(textChanged(QString)), this,
          SLOT(onTimeEditTextChanged(QString)));
  connect(ui->time_box, SIGNAL(currentIndexChanged(int)), this,
          SLOT(onTimeEditIndexChanged()));
  connect(ui->percent_edit, SIGNAL(textChanged(QString)), this,
          SLOT(onInterestTextChanged(QString)));
  connect(ui->depositBox, &QComboBox::currentTextChanged, this,
          &deposit::change_mode);
}
...
int deposit::onAmountEditTextChanged(const QString &text) {
  ...
  } else if (match.hasMatch() && text.toDouble() &&
             !(text[0] == '0' && text[1] != '.')) {
    ui->amount_edit->setStyleSheet(opers->correct_style);
    valid = CORRECT_EXPR;
  } else {
...
int deposit::onInterestTextChanged(const QString &text) {
  ...
  } else if (match.hasMatch() && text.toDouble() && text.toDouble() <= 999 &&
             !(text[0] == '0' && text[1] != '.')) {
    ui->percent_edit->setStyleSheet(opers->correct_style);
    valid = CORRECT_EXPR;
  } else {
...
void deposit::on_calculate_clicked() {
  ...
  if (validation() == 0) {
    ...
    if (opers) {
      ...
      for (int count = 0; count < opers->getTableWidget()->rowCount();
           count++) {
    ...
    if (tableWindow) {
      ...
      tableWindow->setGeometry(currentPosGlobal.x(), currentPosGlobal.y(), 800,
                               600);
      tableWindow->show();
...
int deposit::check_date_between(const QDate &previous, const QDate &current,
                                int *oper_count) {
  QDate one_date;
...
void deposit::add_datarow_to_table(const QDate &date, QString row_head,
                                   long double **result, int iteration,
                                   int style) {
  ...
  for (int j = 0; j < 4; j++) {
    add_item_to_table(iteration, j + 1,
                      QString::number(result[iteration][j], 'f', 2), style);
  }
}
...
void deposit::add_all_to_table(deposit_init data, investment *pay) {
  ...
  for (int i = 0; i < data.current + 1; i++) {
    add_one_period(&begin, &end_period, last_day, data.capital_time,
                   ui->date_edit->date().day());
    ...
    if (oper_count != opers->getTableWidget()->rowCount()) {
      while (check_date_between(prev, current_day, &oper_count)) {
        QTableWidgetItem *redem_item =
            opers->getTableWidget()->item(oper_count, 0);
        QTableWidgetItem *redem_item_1 =
            opers->getTableWidget()->item(oper_count, 1);
        QTableWidgetItem *redem_item_2 =
            opers->getTableWidget()->item(oper_count, 2);
        int style = 1;
    ...
    add_datarow_to_table(current_day, QString::number(i + 1 - oper_count),
                         pay->result, i, 0);
  ...
  for (int i = 0; i < 2; i++) {
    QString number =
        QString::number(static_cast<double>(pay->total[i]), 'f', 2);
    if (i == 0) {
  ...
  for (int i = 0; i < pay->taxes_count; i++) {
    ...
    for (int j = 0; j < 6; j++) {
      ...
      if (j == 5) {
        if (pay->taxes[i][4]) {
          item_text = "Pay by 01.12." +
                      QString::number(static_cast<double>(pay->taxes[i][0] + 1),
                                      'f', 0);
        }
      } else {
        item_text = QString::number(static_cast<double>(pay->taxes[i][j]), 'f',
                                    (!j) ? 0 : 2);
      }
  ...
  if (data.capital) {
    ...
    effective_rate = pay->total[0] / ui->amount_edit->text().toDouble() * 365 /
                     ui->date_edit->date().daysTo(end_deposit) * 100;
    tableWindow->getUi()->total_info->setText(
        "Effective rate: " +
        QString::number(static_cast<double>(effective_rate), 'f', 2) + " %");
  ...
  tableWindow->getUi()->info_rate->setText(
      "Tax: " + QString::number(static_cast<double>(all_tax), 'f', 2));
  tableWindow->getUi()->info_gains->setText(
      "Capital gains: " +
      QString::number(
          static_cast<double>(pay->total[0] /
                              ui->amount_edit->text().toDouble() * 100),
          'f', 2) +
      " %");
  tableWindow->getUi()->info_tax->setText(
      "Gain with tax: " +
      QString::number(static_cast<double>(pay->total[0] - all_tax), 'f', 2));
  tableWindow->getUi()->info_refills->setText(
      "Refills: " + QString::number(static_cast<double>(all_refill), 'f', 2));
  tableWindow->getUi()->info_withdrawals->setText(
      "Withdrawals: " +
      QString::number(static_cast<double>(all_withdrawals), 'f', 2));
  tableWindow->getUi()->info_fullsum->setText(
      "Amount+interest: " +
      QString::number(static_cast<double>(deposit_with_profit), 'f', 2));
}
