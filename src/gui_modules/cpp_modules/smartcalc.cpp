...
smartcalc::smartcalc(QWidget *parent)
  ...
  listView->setStyleSheet(
      "QListView {background-color: rgb(30, 27, 6);} QListView::item "
      "{border-bottom: 5px solid rgb(30, 27, 6); margin:3px; background-color: "
      "rgb(30, 27, 6); } QListView::item:selected {border-bottom: 5px solid "
      "rgb(175, 97, 33); margin:3px; color: white;}");
  ui->switch_window->setView(listView);

  ui->history_info_label->setVisible(false);
  ui->history_widget->setVisible(false);

  connect(this, &smartcalc::resized, this, &smartcalc::onResized);
  connect(ui->result, &QLineEdit::textChanged, this, &smartcalc::onCheckExpr);
  connect(ui->switch_window, &QComboBox::currentTextChanged, this,
          &smartcalc::change_mode);
  connect(this, &smartcalc::keyPressed, this, &smartcalc::handleKeyPress);
}

void smartcalc::change_mode(const QString index) {
  QPoint currentPosGlobal = this->mapToGlobal(QPoint(0, 0));
  QSize currentSize = this->size();
  QMainWindow *newWindow = nullptr;

  if (index == "Deposit") {
    newWindow = new deposit();
  ...
  if (newWindow) {
    this->close();
    newWindow->setGeometry(currentPosGlobal.x(), currentPosGlobal.y(),
...
void smartcalc::handleKeyPress(int key) {
  if (key >= 48 && key <= 57) {
...
int smartcalc::additional_validation(QString text) {
  text.remove(' ');
  int count_open = 0;
  int count_close = 0;
  ...
  if (text.length() && begin.count(text[0]) &&
      end.count(text[text.length() - 1])) {
      ...
      if (text[i] == ')') {
        ...
        if (i < text.length() - 1 &&
          ...
          empty_brackets = 1;
                 ...
                 !nums.count(text[i + 1])) {
        valid = WRONG_EXPR;
      } else if (i < text.length() - 1 && lexems.count(text[i]) &&
                 lexems.count(text[i + 1])) {
        empty_brackets = 1;
      }
    }
...
void smartcalc::on_actionVarChanged(const QString &text) {
  static const QRegularExpression regex("^[0-9]{1,12}(\\.[0-9]{1,12})?$");
  QRegularExpressionMatch match = regex.match(text);

  if (text.length() == 0) {
    var->setStyleSheet(QString(
        "QLineEdit {border-radius: 10px;padding: 0 8px;background-color: "
        "rgb(226, 226, 226);color:black;border: 1px solid black;}"));
    correct_var = NO;
    var->setPlaceholderText("x = ");
  } else if (match.hasMatch() &&
             (!text.toDouble() ||
              (text.toDouble() && !(text[0] == '0' && text[1] != '.')))) {
    var->setStyleSheet(QString(
        "QLineEdit {border-radius: 10px;padding: 0 8px;background-color: "
        "rgb(226, 226, 226);color:black;border: 1px solid black;}"));
    correct_var = YES;
    ...
    var->setStyleSheet(QString(
        "QLineEdit {border-radius: 10px;padding: 0 8px;background-color: "
        "rgba(255, 50, 50, 150);color:black;border: 1px solid black;}"));
    correct_var = NO;
  }
}

void smartcalc::onCheckExpr(const QString &text) {
  if (clear_after) {
    ui->result->setText("");
    clear_after = false;
  } else {
    ...
    if (wrong_expression == NO) {
      ...
      if (wrong_expression == NO) {
        ui->result->setStyleSheet(
            QString("background-color: rgb(30, 27, 6); font-size: %1px; "
                    "border: 0px;color: rgb(255, 255, 255);")
                .arg(font_size));
        valid = YES;
      } else {
        ui->result->setStyleSheet(
            QString("background-color: rgb(30, 27, 6); font-size: %1px; "
                    "border: 0px;color: rgba(255, 50, 50, 150);")
                .arg(font_size));
        valid = NO;
      }
    } else {
      ui->result->setStyleSheet(
          QString("background-color: rgb(30, 27, 6); font-size: %1px; border: "
                  "0px;color: rgba(255, 50, 50, 150);")
              .arg(font_size));
      valid = NO;
    }
  }
...
void smartcalc::onResized(const QSize &newSize) {
  int maxWidth = 700;
    ...
    if (button_item) {
      if (QWidget *widget = button_item->widget()) {
        if (QPushButton *butt = qobject_cast<QPushButton *>(widget)) {
          ...
          style += " " +
                   QString::number((font_size > 44) ? 25 : font_size - 19) +
                   "px;}";
  ...
  if (newSize.width() > maxWidth && !ui->history_info_label->isVisible()) {
    ...
    ui->history_widget->setVisible(true);
             ...
             ui->history_info_label->isVisible()) {
    ui->history_info_label->setVisible(false);
    ui->history_widget->setVisible(false);
  }
}

void smartcalc::resizeEvent(QResizeEvent *event) {
...
void smartcalc::on_graph_clicked() {
  if (!graphWindow) {
    QPoint currentPosGlobal = this->mapToGlobal(QPoint(-600, 0));
    graphWindow = new graphics();
    graphWindow->setGeometry(currentPosGlobal.x(), currentPosGlobal.y(), 600,
                             600);
    QIcon icon(":plot.png");
    graphWindow->setWindowIcon(icon);
    graphWindow->show();
    connect(graphWindow, SIGNAL(graphWindowClosed()), this,
            SLOT(on_graphWindowClosed()));
    ...
    disconnect(pushButton, &QPushButton::clicked, this,
               &smartcalc::on_push_eq_clicked);
    connect(pushButton, &QPushButton::clicked, this,
            &smartcalc::on_actionPlotTriggered);
  } else {
    graphWindow->close();
  }
}

void smartcalc::on_graphWindowClosed() {
  if (!var) {
    if (pushButton) {
      delete pushButton;
      pushButton = nullptr;
...
void smartcalc::switch_buttons(QString name1, QString name2) {
  if (!pushButton) {
  ...
  if (!pushButton1) {
    ...
    connect(pushButton1, &QPushButton::clicked, this,
            &smartcalc::on_actionVarTriggered);
  }
  change_color(ui->push_eq, "black_eq");
}

void smartcalc::createPlotButton(QPushButton *button, int row) {
  button->setMinimumSize(80, 50);
  button->setSizePolicy(QSizePolicy::Minimum, QSizePolicy::Minimum);
  ui->buttons->addWidget(button, row, 4, 1, 1);
}

void smartcalc::on_actionPlotTriggered() {
...
void smartcalc::on_variable_clicked() {
  if (!var) {
    ...
    connect(var, &QLineEdit::textChanged, this,
            &smartcalc::on_actionVarChanged);
  } else {
    disconnect(var, &QLineEdit::textChanged, this,
               &smartcalc::on_actionVarChanged);
...
void smartcalc::change_color(QPushButton *button, QString color) {
  ...
  if (color == "orange_eq") {
    button->setStyleSheet(
        "QPushButton { background-color: rgb(175, 97, 33); color: rgb(255, "
        "255, 255); font-size: " +
        QString::number(fontSize) +
        "px; } QPushButton:pressed { background-color: rgb(50, 50, 50); }");
  } else if (color == "orange") {
    button->setStyleSheet(
        "QPushButton { background-color: rgb(175, 97, 33); color: rgb(255, "
        "255, 255);} QPushButton:pressed { background-color: rgb(50, 50, 50); "
        "} QToolTip { background-color: rgb(226, 226, 226); border: 1px solid "
        "white; color: rgb(0, 0, 0); }");
  } else if (color == "blue") {
    button->setStyleSheet(
        "QPushButton { background-color: rgb(0, 119, 171); color: rgb(255, "
        "255, 255); font-size: " +
        QString::number(fontSize) +
        "px; } QPushButton:pressed { background-color: rgb(175, 97, 33); }");
  } else if (color == "blue_graph") {
    button->setStyleSheet(
        "QPushButton { background-color: rgb(0, 119, 171); color: rgb(255, "
        "255, 255);} QPushButton:pressed { background-color: rgb(175, 97, 33); "
        "} QToolTip { background-color: rgb(226, 226, 226); border: 1px solid "
        "white; color: rgb(0, 0, 0); }");
  } else if (color == "blue_eq") {
    button->setStyleSheet(
        "QPushButton { background-color: rgb(23, 135, 21); color: rgb(255, "
        "255, 255);} QPushButton:pressed { background-color: rgb(50, 50, "
        "50);}");
  } else if (color == "green") {
    button->setStyleSheet(
        "QPushButton { background-color: rgb(23, 135, 21); color: rgb(255, "
    ...
    button->setStyleSheet(
        "QPushButton { background-color: rgb(30, 27, 6); color: rgb(30, 27, 6) "
        "} QPushButton:pressed { background-color: rgb(50, 50, 50); }");
  }
}

void smartcalc::clear_history(int key) {
  int list_size = history.size();
  if (key) {
    for (int i = 0; i < list_size - history_count - 1; i++) {
      history.removeAt(history_count + 1);
      QListWidgetItem *itemToRemove =
          ui->history_widget->takeItem(history_count + 1);
      delete itemToRemove;
    }
  } else {
...
void smartcalc::on_turn_back_clicked() {
  clear_after = false;
  if (history_count > 0) {
    if (!(history_count + 1 == history.size() &&
          ui->result->text() != history[history_count]))
      history_count--;
  }
  if (history.size()) {
...
void smartcalc::on_move_frwd_clicked() {
  if (history_count >= -1) {
...
void smartcalc::on_push_del_clicked() {
  if (ui->result->text().length() > 0) {
...
void smartcalc::on_push_unar_clicked() {
  if (ui->result->text().length() > 0) {
    QString field = ui->result->text();
    QByteArray byteArray = field.toUtf8();
    char *charArray = byteArray.data();
...
void smartcalc::add(QString text) {
...
void smartcalc::on_actionVarTriggered() { add("x"); }

void smartcalc::on_push_sqrt_clicked() { add("sqrt("); }

void smartcalc::on_push_log_clicked() { add("log("); }

void smartcalc::on_push_ln_clicked() { add("ln("); }

void smartcalc::on_push_sin_clicked() { add("sin("); }

void smartcalc::on_push_cos_clicked() { add("cos("); }

void smartcalc::on_push_tan_clicked() { add("tan("); }

void smartcalc::on_push_asin_clicked() { add("asin("); }

void smartcalc::on_push_acos_clicked() { add("acos("); }

void smartcalc::on_push_atan_clicked() { add("atan("); }

void smartcalc::on_push_mod_clicked() {
  if (ui->result->text().length() > 0) {
    add(" mod ");
  }
}

void smartcalc::on_push_opn_brack_clicked() { add("("); }

void smartcalc::on_push_cls_brack_clicked() {
  if (ui->result->text().length() > 0) {
    add(")");
  }
}
...
void smartcalc::on_push_mul_clicked() {
  if (ui->result->text().length() > 0) {
    add("*");
  }
}
...
void smartcalc::on_push_e_clicked() { add("e"); }

void smartcalc::on_push_pi_clicked() { add("P"); }

void smartcalc::on_push_exp_clicked() {
  if (ui->result->text().length() > 0) {
    add("^");
  }
}

void smartcalc::on_push_c_clicked() { ui->result->setText(""); }
