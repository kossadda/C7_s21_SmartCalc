 ...
 * @brief The module contains all functionality for calculating
 * refills/withdrawals
 * @version 1.0
 * @date 2024-01-15
 *
...
static long double calc_period_percent(deposit_init *data, time_data *begin,
                                       time_data *oper, time_data *end);
 ...
 * @param[out] pay structure containing buffer variables for period results and
 * general payment data arrays.
 * @param[out] oper structure containing data on deposit/withdrawal operations.
 * @param[out] end_period structure containing the payment end date for the
 * current period.
 * @param[out] percent the amount accumulated before the expected
 * replenishment/withdrawal in the current period.
 *
...
int check_operation(deposit_init *data, investment *pay, operations *oper,
  ...
  if (oper && oper->count && oper->current < oper->count) {
    while (
      ...
      if ((data->amount < oper_sum ||
           data->amount - oper_sum < oper->min_balance) &&
          oper->type[oper->current] == WITHDRAWALS) {
        pay->balance_changing = 0;
      } else {
        *percent += calc_period_percent(
            data, &data->date, &(oper->date[oper->current]), end_period);
...
/*!
 * @brief Calculates and records the transaction, remembering the accumulated
 * amount before the transaction.
 *
 * @param[out] data structure containing input parameters for calculation.
 * @param[out] begin structure containing information about the beginning of the
 * current period.
 * @param[out] operstructure containing information about the operation.
 * @param[out] end structure containing information about the end of the current
 * period.
 *
 * @return long double - the amount accumulated before the operation.
 */
static long double calc_period_percent(deposit_init *data, time_data *begin,
                                       time_data *oper, time_data *end) {
  ...
  percent =
      percent_formula(data->amount, data->rate, begin->leap, begin->month_days);
  percent +=
      percent_formula(data->amount, data->rate, oper->leap, oper->month_days);
