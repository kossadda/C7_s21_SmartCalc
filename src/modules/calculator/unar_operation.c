/**
 * @file unar_operation.c
 * @author kossadda (https://github.com/kossadda)
 * @brief The module contains additional functions for processing the unary
 * minus at the front.
 * @version 1.0
 * @date 2024-01-15
 *
...
/*!
 * @brief Function for adding/removing a unary minus from the last number in a
 * line.
 *
 ...
 *
 * @return int - index of the character from which the number begins.
 */
 ...
 * @param[in] len string length.
 *
 * @return int - index of the last character containing a minus value.
 */
int minus_position(const char *str, int len) {
  int pos_minus = 0;
  for (int i = len; i >= 0; i--) {
    if (str[i] == CHAR_MINUS && str[i - 1] == CHAR_OP_BRCK &&
        str[len] == CHAR_CL_BRCK) {
      pos_minus = i;
      i = -1;
    }
  }
 ...
 */
void make_unar(const char *str, char *ptr, int len, int pos_number) {
  int count = 0;
  if (pos_number == 0) {
    pos_number = -1;
  }
 ...
 */
void clean_unar(const char *str, char *ptr, int len, int pos_minus) {
  int count = 0;
  for (int i = 0; i < len; i++) {
