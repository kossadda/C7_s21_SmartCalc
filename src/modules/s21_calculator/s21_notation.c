 ...
 * @brief The module contains the basic elements of the notation
 * @version 1.0
 * @date 2024-01-15
 *
...
/*!
 * @brief String parsing function to determine priority and then evaluate
    ...
    if (check(str[i], ALL_OPERATIONS)) {
      ...
      if (str[i] == CHAR_CL_BRCK && lex.stack[lex.count] == CHAR_OP_BRCK) {
        ...
        if (check(lex.stack[lex.count], TRIGONOMETRIC_CHARS)) {
          math_trigonometry(&(num.stack[num.count - 1]),
                            lex.stack[lex.count--]);
        }
      }
...
/*!
 * @brief Function for clearing the top of stacks. Function have two modes.
