 ...
 * @param[in] pay structure containing buffer variables for monthly results and
...
void input_redemption(early_pay *redemption, int day, int month, int year,
  ...
  redemption->date = (time_data *)realloc(
      redemption->date, (redemption->count + 1) * sizeof(time_data));
  redemption->sum = (long double *)realloc(
      redemption->sum, (redemption->count + 1) * sizeof(long double));
  redemption->type =
      (int *)realloc(redemption->type, (redemption->count + 1) * sizeof(int));
 ...
 * @param capital Deposit capitalization enabled/disabled. NOT_CAPIT = 0 -
 * disabled, CAPIT = 1 - enabled.
 */
...
/**
 * @brief Function for filling in the fields for initializing refill/withdrawals
 * arrays.
 *
 * @param[in] oper structure containing data on refill/withdrawals.
 ...
 * @param min_balance the amount below which the balance cannot fall due to
 * withdrawals.
 */
void input_operation(operations *oper, int day, int month, int year,
                     long double sum, int type, long double min_balance) {
  oper->date =
      (time_data *)realloc(oper->date, (oper->count + 1) * sizeof(time_data));
  oper->sum = (long double *)realloc(oper->sum,
                                     (oper->count + 1) * sizeof(long double));
